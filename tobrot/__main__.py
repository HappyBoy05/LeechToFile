...
#    app.add_handler(incoming_size_checker_handler)
    ...
    rename_message_handler = MessageHandler(
        rename_message_f,
        filters=filters.command(["rename@uploadinghb4allbot"]) & filters.chat(chats=AUTH_CHANNEL)
    )
    app.add_handler(rename_message_handler)
    #
    upload_document_handler = MessageHandler(
        upload_document_f,
        filters=filters.command(["upload@uploadinghb4allbot"]) & filters.chat(chats=AUTH_CHANNEL)
    )
    ...
    save_thumb_nail_handler = MessageHandler(
        save_thumb_nail,
        filters=filters.command([f"{SAVETHUMBNAIL@uploadinghb4all}"]) & filters.chat(chats=AUTH_CHANNEL)
    )
    app.add_handler(save_thumb_nail_handler)
    #
    clear_thumb_nail_handler = MessageHandler(
        clear_thumb_nail,
        filters=filters.command([f"{CLEARTHUMBNAIL@uploadinghb4all}"]) & filters.chat(chats=AUTH_CHANNEL)
    )
